...
//            0x10u -> STOP()
            ...
            0xE0u -> `LDH (a8), A`()
            0xF0u -> `LDH A, (a8)`()
            0x02u, 0x12u, 0x22u, 0x32u -> `LD (nn), A`(opcode)
            0x0Au, 0x1Au, 0x2Au, 0x3Au -> `LD A, (nn)`(opcode)
...
//                in 0x38u..0x3Fu -> `SRL n`(cbOp)
    ...
    private fun `LD (nn), A`(opcode: UInt) {
        when(opcode) {
            0x02u -> memory[BC] = A
            0x12u -> memory[DE] = A
            0x22u -> memory[HL++] = A
    ...
    private fun `LD A, (nn)`(opcode: UInt) {
        when(opcode) {
            0x0Au -> A = memory[BC]
            0x1Au -> A = memory[DE]
            0x2Au -> A = memory[HL++]
    ...
    private fun `SUB A, r`(opcode: UInt) {
        val offset = opcode % 8u
        val result = A - readRegisterBy(offset)
        A = result.toUByte()
        zeroBit = A == UByte.ZERO
        carryBit = result.bit(7)
        halfCarryBit = result.bit(3)
        subtractBit = true
    }
